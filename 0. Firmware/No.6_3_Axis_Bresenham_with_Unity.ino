...
void SettingDef() {  // Pre-set the default microstep setting
  ...
  if (microstepState == 1 || microstepState == 2 || microstepState == 4 || microstepState == 8 || microstepState == 16 || microstepState == 32) {
    ...
    stepsForMM = (stepsForRev_Micro * revForMM);
  ...
  if ((command = Finding('G', noLetr)) && (Finding('M', noLetr) == noLetr) && ((Finding('G', noLetr) != 0) || (Finding('G', noLetr) != 1))) {  // After finding 'G', the return value will be stored in 'command'.
    switch (command) {
      ...
      case STATE1_AUTO_CALIBRATION:
        ...
        break;
      case STATE1_MANUALLY_SET_POSITON:
        ManualSet();
        break;
...
double Finding(char letter, double medium) {
  ...
  while ((long)pointer > 1 && (*pointer) && (long)pointer < ((long)array + curBuf)) {  // Refers to (https://github.com/MarginallyClever/GcodeCNCDemo)
...
void Calibrating() {
  ...
  maxDisX = (abs(maxPosX) - 2 * safetyDis) / stepsForMM;
  ...
  maxDisY = (abs(maxPosY) - 2 * safetyDisR) / stepsForMM;
  ...
  maxDisZ = (abs(maxPosZ) - 2 * safetyDisR) / stepsForMM;
...
void AbsLinPos() {
  ...
  if (Finding('X', noLetr) == noLetr) {
    ...
    endPosX = (-1) * Finding('X', noLetr) * stepsForMM;
  }
  ...
  if (Finding('Y', noLetr) == noLetr) {
    ...
    endPosY = Finding('Y', noLetr) * stepsForMM;
  }
  ...
  if (Finding('Z', noLetr) == noLetr) {
    ...
    endPosZ = (Finding('Z', noLetr) * stepsForMM);
  }
  if (endPosZ > maxPosZ) {
...
void RelLinPos() {
  ...
  if (Finding('X', noLetr) && retMed == 0) {
    ...
    endPosX = (initPosX + (-Finding('X', noLetr) * stepsForMM));
  }
  ...
  if (Finding('Y', noLetr) && retMed == 0) {
    ...
    endPosY = (initPosY + (Finding('Y', noLetr) * stepsForMM));
  }
  ...
  if (Finding('Z', noLetr) && retMed == 0) {
    ...
    endPosZ = (initPosZ + (Finding('Z', noLetr) * stepsForMM));
  }
...
void RelArcPosCW() {
  initPosX = (stepper1.currentPosition() / stepsForMM), initPosY = (stepper2.currentPosition() / stepsForMM), initPosYR = (-1) * initPosY;
  CenterPos();
