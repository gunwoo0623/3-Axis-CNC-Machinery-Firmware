...
long maxDis1X = 0, maxDis1Y = 0, maxDis1Z = 0;
long maxDis2X = 0, maxDis2Y = 0, maxDis2Z = 0;
...
float stepperSpeedZ = 0;        // Because of how the stepper motor is set. Put the minus in front of the value of 'stepperSpeed'
...
void HomingBackY() {
  ...
  Serial.println(abs(stepper2.currentPosition()));
...
void HomingBackZ() {
  ...
  Serial.println(abs(stepper3.currentPosition()));
...
void Calibrating() {
  ...
  // Serial.print(F("   Two measured maximum distances: "));
  // Serial.print(abs(maxDis1Y));
  // Serial.print(F(", "));
  // Serial.println(abs(maxDis2Y));
  // Serial.print(F("   The difference between two calibrations: "));
  // Serial.println(maxDis1Y + maxDis2Y);
  Serial.print(F("   The average maximum distance on the Y-Axis: "));
  maxDisY = -1 * ((abs(maxDis1Y) + abs(maxDis2Y)) / 2);
  Serial.println(abs(maxDisY));
  ...
  // Serial.print(F("   Two measured maximum distances: "));
  // Serial.print(abs(maxDis1Z));
  // Serial.print(F(", "));
  // Serial.println(abs(maxDis2Z));
  // Serial.print(F("   The difference between two calibrations: "));
  // Serial.println(maxDis1Z + maxDis2Z);
  Serial.print(F("   The average maximum distance on the Z-Axis: "));
  maxDisZ = ((abs(maxDis1Z) + abs(maxDis2Z)) / 2);
  Serial.println(abs(maxDisZ));
  Serial.println(F(""));
}
...
void AbsolutePosX() {
  ...
  Serial.print(abs(maxDisX));
  Serial.println(F("]"));

  Serial.print(F("   X-Axis: "));
  Serial.parseInt();  // Clear the buffer
  while (Serial.available() == 0) {}
  targetPosX = (-1 * Serial.parseInt());
  Serial.println(abs(targetPosX));
...
void AbsolutePosY() {
  ...
  Serial.print(abs(maxDisY));
  Serial.println(F("]"));

  Serial.print(F("   Y-Axis: "));
  Serial.parseInt();  // Clear the buffer
  while (Serial.available() == 0) {}
  targetPosY = (-1 * Serial.parseInt());
  Serial.println(abs(targetPosY));
...
void AbsolutePosZ() {
  ...
  Serial.print(maxDisZ);
  Serial.println(F("]"));

  Serial.print(F("   Z-Axis: "));
  Serial.parseInt();  // Clear the buffer
  while (Serial.available() == 0) {}
  targetPosZ = Serial.parseInt();
  ...
  if (((targetPosX == prevPosX) || (targetPosX == 0)) && ((targetPosY == prevPosY) || (targetPosY == 0))) {
    ...
    if ((stepper1.currentPosition() == targetPosX) || (targetPosX == 0)) {  // the second limit-switch detected is X-axis one
      ...
      if ((targetPosY == prevPosY) || (targetPosY == 0)) {
        ...
        Serial.print(abs(stepper2.currentPosition()));
        Serial.print(F("/"));
        Serial.println(abs(maxDisY));
      }
...
JumpToZ:
  if ((targetPosZ == prevPosZ) || (targetPosZ == 0)) {
    ...
    Serial.print(abs(stepper3.currentPosition()));
    Serial.print(F("/"));
    Serial.println(maxDisZ);
    Serial.println(F(""));
  }
  Serial.print(F(""));
}
...
void Rewinding() {  // The reason why it needs to go back to the origin is to reset and know where this motor was actually
  ...
  Serial.print(abs(prevPosX));
  Serial.print(F(", "));
  Serial.print(abs(prevPosY));
  Serial.print(F(", "));
  Serial.print(abs(prevPosZ));
  Serial.println(F("]"));
